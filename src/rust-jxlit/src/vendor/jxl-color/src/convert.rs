...
impl ColorTransform {
    ...
    /// Returns whether every op in this transform has a GPU implementation.
    #[inline]
    pub fn is_gpu_supported(&self) -> bool {
        self.ops
            .iter()
            .all(|op| !matches!(op, ColorTransformOp::IccToIcc(_)))
    }

    /// Serializes the transform into GPU-dispatchable ops.
    ///
    /// Returns [`GpuTransformUnsupported::IccToIcc`] when the plan contains a CMS transform.
    pub fn gpu_ops(
        &self,
    ) -> std::result::Result<Vec<ColorTransformGpuOp>, GpuTransformUnsupported> {
        self.ops.iter().map(ColorTransformOp::to_gpu_op).collect()
    }
}

/// Reason a color transform cannot run on GPU.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum GpuTransformUnsupported {
    IccToIcc,
}

/// GPU-serializable color transform operation (excludes ICC CMS handles).
#[derive(Debug, Clone, PartialEq)]
pub enum ColorTransformGpuOp {
    XybToMixedLms {
        opsin_bias: [f32; 3],
...
impl ColorTransformOp {
    fn to_gpu_op(&self) -> std::result::Result<ColorTransformGpuOp, GpuTransformUnsupported> {
        Ok(match self {
            Self::IccToIcc(_) => return Err(GpuTransformUnsupported::IccToIcc),
            Self::XybToMixedLms {
                opsin_bias,
                intensity_target,
            } => ColorTransformGpuOp::XybToMixedLms {
                opsin_bias: *opsin_bias,
                intensity_target: *intensity_target,
            },
            Self::LumaToXyz { illuminant } => ColorTransformGpuOp::LumaToXyz {
                illuminant: *illuminant,
            },
            Self::XyzToLuma => ColorTransformGpuOp::XyzToLuma,
            Self::Matrix(m) => ColorTransformGpuOp::Matrix(*m),
            Self::TransferFunction {
                tf,
                hdr_params,
                inverse,
                ...
                tf: *tf,
                luminances: hdr_params.luminances,
                intensity_target: hdr_params.intensity_target,
                min_nits: hdr_params.min_nits,
                inverse: *inverse,
            },
            Self::HlgInverseOotf(hdr_params) => ColorTransformGpuOp::HlgInverseOotf {
                luminances: hdr_params.luminances,
                intensity_target: hdr_params.intensity_target,
                min_nits: hdr_params.min_nits,
            },
            ...
            Self::Clip => ColorTransformGpuOp::Clip,
        })
    }
