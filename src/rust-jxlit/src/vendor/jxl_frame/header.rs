...
// Source: https://github.com/tirr-c/jxl-oxide/blob/f8ae722ef2d6b782941c89517d19cfbf605c4a9d/crates/jxl-frame/src/header.rs
// Copied as-is; only crate-path references changed.

use crate::vendor::jxl_frame::Result;
...
define_bundle! {
    /// Frame header.
    #[derive(Debug)]
    pub struct FrameHeader ctx(headers: &ImageHeader) error(crate::vendor::jxl_frame::Error) {
    ...
    #[derive(Debug)]
    pub struct Passes error(crate::vendor::jxl_frame::Error) {
    ...
    #[derive(Debug)]
    pub struct BlendingInfo ctx(context: (bool, Option<BlendMode>, CanvasSizeParams<'_>)) error(crate::vendor::jxl_frame::Error) {
    ...
    #[derive(Debug)]
    pub struct RestorationFilter ctx(encoding: Encoding) error(crate::vendor::jxl_frame::Error) {
        all_default: ty(Bool) default(true),
        pub gab: ty(Bundle(crate::vendor::jxl_frame::filter::Gabor)) cond(!all_default),
        pub epf: ty(Bundle(crate::vendor::jxl_frame::filter::EdgePreservingFilter)) cond(!all_default),
        pub extensions: ty(Bundle(Extensions)) cond(!all_default),
    }
}
