...
// Source: https://github.com/tirr-c/jxl-oxide/blob/f8ae722ef2d6b782941c89517d19cfbf605c4a9d/crates/jxl-render/src/image.rs
// Copied as-is; only crate-path references changed.

use std::sync::Arc;

use crate::vendor::jxl_frame::{FrameHeader, data::GlobalModular};
use crate::vendor::jxl_vardct::LfChannelDequantization;
...
use crate::vendor::jxl_render::{
    FrameRender, FrameRenderHandle, IndexedFrame, Region, Result, util,
};
...
impl ImageWithRegion {
    ...
    pub(crate) fn upsample_jpeg(
        ...
        for (g, (region, shift)) in self.buffer.iter_mut().zip(&mut self.regions).take(3) {
            ...
            let out = crate::vendor::jxl_render::filter::apply_jpeg_upsampling_single(
                subgrid,
                *shift,
                valid_region,
                self.tracker.as_ref(),
    ...
    pub(crate) fn upsample_nonseparable(
        ...
        for (idx, ((region, shift), g)) in self.regions.iter_mut().zip(&mut self.buffer).enumerate()
        {
            ...
            *region = downsampled_valid_region;
            ...
            .in_scope(|| {
                crate::vendor::jxl_render::features::upsample(
                    subgrid,
                    region,
                    image_header,
...
impl ImageWithRegion {
    ...
    pub(crate) fn color_groups_with_group_id(
        &mut self,
        frame_header: &crate::vendor::jxl_frame::FrameHeader,
...
pub(crate) fn composite<S: Sample>(
    frame: &IndexedFrame,
    grid: &mut ImageWithRegion,
    refs: [Option<crate::vendor::jxl_render::Reference<S>>; 4],
    ...
    let image = crate::vendor::jxl_render::blend::blend(
        image_header,
        refs,
        frame,
        grid,
