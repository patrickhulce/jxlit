...
// Source: https://github.com/tirr-c/jxl-oxide/blob/f8ae722ef2d6b782941c89517d19cfbf605c4a9d/crates/jxl-render/src/lib.rs
// Copied as-is; only crate-path references changed.

//! This crate is the core of jxl-oxide that provides JPEG XL renderer.
use std::sync::{Arc, Mutex};

use crate::vendor::jxl_frame::{Frame, FrameContext, header::FrameType};
...
// Submodule visibility relaxed to pub(crate) so the staged decode pipeline can
// drive the same building blocks. Module contents are otherwise verbatim.
pub(crate) mod blend;
mod error;
...
impl RenderContext {
    ...
    #[inline]
    pub(crate) fn narrow_modular(&self) -> bool {
...
impl RenderContext {
    ...
    pub(crate) fn get_previous_frames_visibility<'a>(
        &'a self,
...
impl RenderContext {
    ...
    pub(crate) fn postprocess_keyframe(
        &self,
        frame: &IndexedFrame,
...
// Added (not in upstream): minimal accessors/helpers so the staged decode
// pipeline (see `crate::pipeline`) can drive the same building blocks that
// `render_keyframe`/`render_by_index` use internally. These expose existing
// `pub(crate)` state without altering any decode logic.
impl RenderContext {
    pub(crate) fn pool(&self) -> &JxlThreadPool {
        &self.pool
    }

    pub(crate) fn requested_image_region(&self) -> Region {
        self.requested_image_region
    }

    pub(crate) fn frames(&self) -> &[Arc<IndexedFrame>] {
        &self.frames
    }

    pub(crate) fn keyframe_frame_index(&self, keyframe_idx: usize) -> Option<usize> {
        self.keyframes.get(keyframe_idx).copied()
    }

    pub(crate) fn reference_frames_wide(&self, idx: usize) -> ReferenceFrames<i32> {
        let deps = self.frame_deps[idx];
        ReferenceFrames {
            lf: (deps.lf != usize::MAX).then(|| Reference {
                frame: Arc::clone(&self.frames[deps.lf]),
    ...
    pub(crate) fn reference_frames_narrow(&self, idx: usize) -> ReferenceFrames<i16> {
        let deps = self.frame_deps[idx];
        ReferenceFrames {
            lf: (deps.lf != usize::MAX).then(|| Reference {
                frame: Arc::clone(&self.frames[deps.lf]),
    ...
    /// Injects a staged-decode result as the frame's rendered grid and runs the
    /// vendored blend/compositing, reproducing `render_by_index` exactly.
    pub(crate) fn blend_staged(
        &self,
        idx: usize,
        grid: ImageWithRegion,
    ) -> Result<Arc<ImageWithRegion>> {
        if self.narrow_modular() {
            let handle = &self.renders_narrow[idx];
            *handle.render.lock().unwrap() = FrameRender::Done(grid);
            Arc::clone(handle).run_with_image()?.blend(None, &self.pool)
        } else {
            let handle = &self.renders_wide[idx];
            *handle.render.lock().unwrap() = FrameRender::Done(grid);
            Arc::clone(handle).run_with_image()?.blend(None, &self.pool)
        }
    }
...
pub(crate) struct ReferenceFrames<S: Sample> {
    pub(crate) lf: Option<Reference<S>>,
    pub(crate) refs: [Option<Reference<S>>; 4],
}

#[derive(Debug, Clone)]
pub(crate) struct Reference<S: Sample> {
